...
雷达信号检测问题可以描述为如下两个假设：
+ $H_0$ —— 雷达信号中没有目标存在，即只有噪声。
+ $H_1$ —— 雷达信号中有目标存在，即有反射回波信号，其上叠加噪声。
...
$$
\begin{align} 
H_0 &:\quad x[n] = w[n], \qquad n = 0, 1, 2, \cdots, N-1  \\
H_1 &:\quad x[n] = s[n] + w[n], \qquad n = 0, 1, 2, \cdots, N-1
\end{align}
$$
其中 $s[n]$ 已知，$w[n]$ 是均值为0、方差为 $\sigma^{2}$ 的Gauss白噪声。Neyman-Pearson准则检测器为
$$
\begin{align} 
L(\v{x}) := \frac{p(\v{x} ; H_1)}{p(\v{x} ; H_0)} 
...
\mathop{\gtrless}\limits_{H_0}^{H_1} \gamma 
\end{align}
$$
即
$$
\quad\mark{ T(\v{x}) = \sum_{n=0}^{N-1} s[n] x[n] } \mathop{\gtrless}\limits_{H_0}^{H_1} \frac{1}{2} \sum_{n=0}^{N-1} s^{2}[n] + \sigma^{2} \ln \gamma =: \gamma'\quad
$$
...
$$
\mathrm{SNR} = \frac{\mathbb{E} \left[ y[N-1] ; H_1 \right]}{\mathrm{var} \left( y[N-1] ; H_1 \right)}
= 
$$
...
即
$$
\quad\mark{ T(\v{x}) = \v{x}^{\mathrm{T}} \boldsymbol{C}^{-1} \v{s} } \mathop{\gtrless}\limits_{H_0}^{H_1} \frac{1}{2} \v{s}^{\mathrm{T}} \boldsymbol{C}^{-1} \v{s} + \ln \gamma =: \gamma'\quad
$$
其中 $T(\v{x})$ 是检测统计量，$\gamma'$ 是对应的门限。
...
通信信号检测问题可以描述为如下假设：
+ $H_{0}$ —— 通信信号中发送的是第 0 个符号 $s_{0}[n]$，其上叠加噪声。
+ ……
...
在先验不相同的情况下，直接使用最小错误概率准则。在二元情况下，假定 $P(H_{0}) = p_{0}$，$P(H_{1}) = 1 - p_{0}$，判决器为
$$
\begin{align}
\frac{p(\v{x} \mid H_1)}{p(\v{x} \mid H_0)} &= \frac{\frac{1}{(2\pi \sigma^{2})^{N/2}} \exp \left( -\frac{1}{2\sigma^{2}} \sum_{n=0}^{N-1} (x[n] - s_{1}[n])^{2} \right)}{\frac{1}{(2\pi \sigma^{2})^{N/2}} \exp \left( -\frac{1}{2\sigma^{2}} \sum_{n=0}^{N-1} (x[n] - s_{0}[n])^{2} \right)} \\
...
&\mathop{\gtrless}\limits_{H_0}^{H_1} \frac{P(H_0)}{P(H_1)} = \frac{p_{0}}{1 - p_{0}} =: \gamma
\end{align}
$$
即
$$
\left( \sum_{n=0}^{N-1} x[n] s_{1}[n] - \frac{1}{2} \varepsilon_{1} \right) - \left( \sum_{n=0}^{N-1} x[n] s_{0}[n] - \frac{1}{2} \varepsilon_{0} \right) \mathop{\gtrless}\limits_{H_0}^{H_1} \sigma^{2} \ln \gamma =: \gamma'
$$
当 $P(H_{0}) = P(H_{1})$ 时，$\gamma' = 0$，退化为前面等先验的情况。

## 随机信号检测

以上两类问题中，信号 $s[n]$ 是已知的确定性信号；而在随机信号检测问题中，信号 $s[n]$ 是一个随机过程，且其统计特性已知。其简化数学模型为
$$
\begin{align}
H_0 &:\quad x[n] = w[n], \qquad n = 0, 1, 2, \cdots, N-1 \\
H_1 &:\quad x[n] = s[n] + w[n], \qquad n = 0, 1, 2, \cdots, N-1
...
$$
\v{x} \sim \begin{cases}
\mathcal{N}(\v{0}, \sigma^{2} \boldsymbol{I}), & H_0 \\
\mathcal{N}(\v{0}, \boldsymbol{C}_{s} + \sigma^{2} \boldsymbol{I}), & H_1
\end{cases}
$$
...
\begin{align}
&& L(\v{x}) := \frac{p(\v{x} ; H_1)}{p(\v{x} ; H_0)} 
= \frac{\frac{1}{(2\pi)^{N/2} \left\lvert \boldsymbol{C}_{s} + \sigma^{2} \boldsymbol{I} \right\rvert^{1/2}} \exp \left( -\frac{1}{2} \v{x}^{\mathrm{T}} (\boldsymbol{C}_{s} + \sigma^{2} \boldsymbol{I})^{-1} \v{x} \right)}{\frac{1}{(2\pi \sigma^{2})^{N/2}} \exp \left( -\frac{1}{2\sigma^{2}} \v{x}^{\mathrm{T}} \v{x} \right)} &\mathop{\gtrless}\limits_{H_0}^{H_1} \gamma \\
\implies \hspace{-4em} && -\frac{1}{2} \v{x}^{\mathrm{T}} \left( (\boldsymbol{C}_{s} + \sigma^{2} \boldsymbol{I})^{-1} - \frac{1}{\sigma^{2}} \boldsymbol{I} \right) \v{x} &\mathop{\gtrless}\limits_{H_0}^{H_1} \gamma'  \\
...
= \v{x}^{\mathrm{T}} \boldsymbol{C}_{sx} \boldsymbol{C}_{xx}^{-1} \v{x}
