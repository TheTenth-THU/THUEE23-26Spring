...
$$
H_{0}:\quad x[n] = w[n], \qquad n = 0, 1, 2, \cdots, N-1
$$
而「有信号」的情况为**备择假设**
$$
H_{1}:\quad x[n] = s[n] + w[n], \qquad n = 0, 1, 2, \cdots, N-1
$$
其中 $s[n]$ 是信号，$w[n]$ 是噪声。这样，可以定义以下概率描述检测的性能：
+ **虚警概率** $P_{\mathrm{FA}} = P(H_1 ; H_0)$，即在零假设 $H_0$ 成立的前提下，检测器错误地判定为备择假设 $H_1$ 的概率；
+ **漏检概率** $P_{\mathrm{M}} = P(H_0 ; H_1)$，即在备择假设 $H_1$ 成立的前提下，检测器错误地判定为零假设 $H_0$ 的概率；
+ **检出概率** $P_{\mathrm{D}} = P(H_1 ; H_1) = 1 - P_{\mathrm{M}}$，即在备择假设 $H_1$ 成立的前提下，检测器正确地判定为备择假设 $H_1$ 的概率。
...
$$
J = P_{\mathrm{D}} + \lambda (P_{\mathrm{FA}} - \alpha) = \dint_{R_{1}} p(\v{x} ; H_1) \dif \v{x} + \lambda \left( \dint_{R_{1}} p(\v{x} ; H_0) \dif \v{x} - \alpha \right)
$$
其中 $R_{1}$ 是判定为备择假设 $H_1$ 的区域。显然只要 $p(\v{x} ; H_1) + \lambda p(\v{x} ; H_0) > 0$，将此点纳入 $R_{1}$ 就可使 $J$ 增大，因此最优的检测器为**似然比检验 (likelihood ratio test, LRT)**
$$
\mark{ L(x) := \frac{p(\v{x} ; H_1)}{p(\v{x} ; H_0)} \mathop{\gtrless}\limits_{H_0}^{H_1} -\lambda =: \gamma }
$$
其中门限 $\gamma$ 可以通过虚警概率 $P_{\mathrm{FA}} = \alpha$ 来确定，即
$$
P_{\mathrm{FA}} = \int_{L(\v{x}) > \gamma} p(\v{x} ; H_0) \dif \v{x} = \alpha
$$
...
> \begin{align}
> & H_{0}:\quad x[n] = w[n], \qquad n = 0, 1, 2, \cdots, N-1 \\
> & H_{1}:\quad x[n] = A + w[n], \qquad n = 0, 1, 2, \cdots, N-1
> \end{align}
> $$
...
> $$
> \frac{p(\v{x} ; H_1)}{p(\v{x} ; H_0)} = \frac{\prod\limits_{n=0}^{N-1} \frac{1}{\sqrt{2\pi \sigma^{2}}} \exp \left( -\frac{(x[n] - A)^{2}}{2\sigma^{2}} \right)}{\prod\limits_{n=0}^{N-1} \frac{1}{\sqrt{2\pi \sigma^{2}}} \exp \left( -\frac{x^{2}[n]}{2\sigma^{2}} \right)} = \exp \left( \frac{A}{\sigma^{2}} \sum_{n=0}^{N-1} x[n] - \frac{NA^{2}}{2\sigma^{2}} \right) \mathop{\gtrless}\limits_{H_0}^{H_1} \gamma
> $$
> 因此最优的检测器为
> $$
> \frac{1}{N} \sum_{n=0}^{N-1} x[n] \mathop{\gtrless}\limits_{H_0}^{H_1} \frac{A}{2} + \frac{\sigma^{2}}{NA} \ln \gamma =: \gamma'
> $$
> 左侧的统计量称为**检测统计量**，其门限 $\gamma'$ 可整体直接由虚警概率 $P_{\mathrm{FA}}$ 来确定，即
> $$
> P_{\mathrm{FA}} = P \left( \frac{1}{N} \sum_{n=0}^{N-1} x[n] > \gamma' ; H_0 \right) = Q \left( \frac{\gamma'}{\sigma / \sqrt{N}} \right)
> \implies
> \gamma' = \frac{\sigma}{\sqrt{N}} Q^{-1}(P_{\mathrm{FA}})
> $$
> 相应地，检出概率为
> $$
> P_{\mathrm{D}} = P \left( \frac{1}{N} \sum_{n=0}^{N-1} x[n] > \gamma' ; H_1 \right) = Q \left( \frac{\gamma' - A}{\sigma / \sqrt{N}} \right) = Q \left( Q^{-1}(P_{\mathrm{FA}}) - \sqrt{ \frac{NA^{2}}{\sigma^{2}} } \right)
> $$
...
$$
\begin{align} 
P_{\mathrm{e}} &= P(H_1) P(H_0 \mid H_1) + P(H_0) P(H_1 \mid H_0) = P(H_1) P_{\mathrm{M}} + P(H_0) P_{\mathrm{FA}}  \\
&= P(H_1) \int_{R_{0}} p(\v{x} \mid H_1) \dif \v{x} + P(H_0) \int_{R_{1}} p(\v{x} \mid H_0) \dif \v{x} \\
&= P(H_1) \left( 1 - \dint_{R_{1}} p(\v{x} \mid H_1) \dif \v{x} \right) + P(H_0) \dint_{R_{1}} p(\v{x} \mid H_0) \dif \v{x} \\
&= P(H_1) + \dint_{R_{1}} \left(P(H_0) p(\v{x} \mid H_0) - P(H_1) p(\v{x} \mid H_1) \right) \dif \v{x}
\end{align}
$$
对于**判错的代价可量化且相同**的二分类检测问题，可以使用最小错误概率准则，即**最小化错误概率 $P_{\mathrm{e}}$**。显然当 $P(H_0) p(\v{x} \mid H_0) - P(H_1) p(\v{x} \mid H_1) < 0$ 时，将此点纳入 $R_{1}$ 就可使 $P_{\mathrm{e}}$ 减小，因此最优的检测器同样为**似然比检验**
$$
\mark{ \frac{p(\v{x} \mid H_1)}{p(\v{x} \mid H_0)} \mathop{\gtrless}\limits_{H_0}^{H_1} \frac{P(H_0)}{P(H_1)} }
$$
这实际上是**最大后验概率 (MAP) 检测器**，即
$$
\frac{p(\v{x} \mid H_1) P(H_1)}{p(\v{x})} = p(H_{1} \mid \v{x}) \mathop{\gtrless}\limits_{H_0}^{H_1} p(H_{0} \mid \v{x}) = \frac{p(\v{x} \mid H_0) P(H_0)}{p(\v{x})}
$$
若先验概率 $P(H_0)$ 和 $P(H_1)$ 相等，则MAP退化为**最大似然 (maximum likelihood, ML) 检测器**。
...
$$
\begin{align}
R &= P(H_1) \left( C_{11} P(H_1 \mid H_1) + C_{01} P(H_0 \mid H_1) \right) + P(H_0) \left( C_{10} P(H_1 \mid H_0) + C_{00} P(H_0 \mid H_0) \right) \\
&= C_{00} P(H_0) \left( 1 - \dint_{R_{1}} p(\v{x} \mid H_0) \dif \v{x} \right) + C_{10} P(H_0) \dint_{R_{1}} p(\v{x} \mid H_0) \dif \v{x}  \\
&\hspace{1em}+ C_{01} P(H_1) \left( 1 - \dint_{R_{1}} p(\v{x} \mid H_1) \dif \v{x} \right) + C_{11} P(H_1) \dint_{R_{1}} p(\v{x} \mid H_1) \dif \v{x} \\
&= C_{00} P(H_0) + C_{01} P(H_1) + \dint_{R_{1}} \left( (C_{10} - C_{00}) P(H_0) p(\v{x} \mid H_0) - (C_{01} - C_{11}) P(H_1) p(\v{x} \mid H_1) \right) \dif \v{x}
\end{align}
$$
最小Bayes风险准则是**最小化Bayes风险 $R$**，而当 $(C_{10} - C_{00}) P(H_0) p(\v{x} \mid H_0) - (C_{01} - C_{11}) P(H_1) p(\v{x} \mid H_1) < 0$ 时，将此点纳入 $R_{1}$ 就可使 $R$ 减小，又一般有 $C_{10} > C_{00}$、$C_{01} > C_{11}$，因此最优的检测器为
$$
\mark{ \frac{p(\v{x} \mid H_1)}{p(\v{x} \mid H_0)} \mathop{\gtrless}\limits_{H_0}^{H_1} \frac{(C_{10} - C_{00}) P(H_0)}{(C_{01} - C_{11}) P(H_1)} }
$$
