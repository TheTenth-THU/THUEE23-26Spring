...
**{Cramer-Rao|克拉美—罗}下界 (Cramer-Rao lower bound, CRLB)** 是满足一定正则条件的任何无偏估计的方差的下界，其给定了无偏估计的方差的理论最小值。
...
考虑测量系统
$$
x[n] = \sum\limits_{k=1}^{M} a_{k} \cos\left( \frac{2\pi kn}{N} \right) + \sum\limits_{k=1}^{M} b_{k} \sin(\frac{2\pi kn}{N}) + w[n], \qquad n = 0, 1, \cdots, N-1
$$
假定
