...
**{Cramer-Rao|克拉美罗}下界 (Cramer-Rao lower bound, CRLB)** 是满足一定正则条件的任何无偏估计的方差的下界，其给定了无偏估计的方差的理论最小值。
...
> \end{pmatrix}
> $$
> 因而其 MVU 估计量为 $\hat{\v{\theta}} = (\boldsymbol{H}^{\mathrm{T}} \boldsymbol{H})^{-1} \boldsymbol{H}^{\mathrm{T}} \v{x}$，协方差矩阵为 $\boldsymbol{C}_{\hat{\v{\theta}}} = \sigma^{2} (\boldsymbol{H}^{\mathrm{T}} \boldsymbol{H})^{-1}$。计算得到
> $$
> \begin{align}
